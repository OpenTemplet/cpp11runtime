...
/*$TET$L$op*/
	if (ts[pid] < T){
		oper(mN, ts[pid], pid); ts[pid]++; duration(_assemble, 1.0);
		return true;
	}
	return false;
/*$TET$*/
...
#ifdef _TEMPLET_SIM
	double T1 = 0.0; double Tp = 0.0; int Pmax = 0; double Smax = 0.0; double Sp = 0.0;
	TEMPLET_SIM::stat(&e, T1, Tp, Pmax, Smax, P, Sp);
